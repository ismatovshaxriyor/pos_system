...
export const BottomNav: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center justify-around px-3 py-2 w-[92%] max-w-md bg-[#001712]/80 dark:bg-[#002019]/80 backdrop-blur-2xl border border-[#E3C282]/35 shadow-2xl rounded-full transition-all">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`relative flex items-center justify-center gap-2 px-5 py-2.5 rounded-full transition-all duration-300 active:scale-95 ${
              isActive
          ...
          >
            <span
              className={`material-symbols-outlined text-xl transition-all ${isActive ? 'filled' : ''}`}
            >
              {item.icon}
            </span>
            <span className="font-sans-body text-xs font-semibold tracking-wider uppercase">
              {item.label}
            </span>
