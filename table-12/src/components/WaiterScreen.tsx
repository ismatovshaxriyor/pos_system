...
export const WaiterScreen: React.FC = () => {
  ...
  return (
    <div className="pt-24 pb-36 px-6 max-w-xl mx-auto min-h-screen animate-in fade-in duration-300">
      ...
      <div className="relative glass-card p-6 rounded-2xl overflow-hidden mb-10 border border-[#0077CC]/30">
        <div className="absolute top-0 right-0 w-14 h-14 bg-[#0077CC]/20 rounded-bl-3xl border-b border-l border-[#0077CC]/40 flex items-center justify-center">
          <span className="material-symbols-outlined text-[#0077CC] text-lg" style={{ fontVariationSettings: "'FILL' 1" }}>
            wine_bar
          </span>
        </div>
        <p className="font-sans-body text-[10px] font-bold tracking-widest text-[#0077CC] mb-3 uppercase">
          {t.sommelierNote}
        </p>
        <h4 className="font-serif-display font-semibold text-xl text-[#FFFFFF] mb-2">
          {t.exclusivePairing}
        </h4>
        <p className="font-sans-body text-xs text-[#9FB0C4] leading-relaxed italic mb-5">
          {t.exclusivePairingText}
        </p>
        <button
          onClick={() => callWaiter('Sommelier Wine Pairing requested')}
          className="font-sans-body text-[11px] font-bold tracking-widest border border-[#0077CC] text-[#0077CC] px-5 py-2 rounded-full hover:bg-[#0077CC] hover:text-white transition-all uppercase active:scale-95"
        >
          {t.requestPairing}
        </button>
      </div>
