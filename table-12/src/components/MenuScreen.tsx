...
export const MenuScreen: React.FC = () => {
  ...
  return (
    <div className="pt-20 pb-40 px-6 md:px-16 max-w-[1440px] mx-auto min-h-screen animate-in fade-in duration-300">
      {/* Sticky Search Bar */}
      <div className="sticky top-16 z-40 bg-[#0A1F44]/90 backdrop-blur-md py-4 -mx-6 px-6 md:-mx-16 md:px-16">
        <div className="relative w-full max-w-2xl mx-auto">
          <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-[#9FB0C4]">
            search
          </span>
          <input
            type="text"
            ...
            className="w-full bg-[#0F2A5C] border-b border-[#0077CC]/30 focus:border-[#0077CC] focus:outline-none text-sm text-[#FFFFFF] py-3.5 pl-12 pr-4 transition-all rounded-t-lg font-sans-body"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-4 top-1/2 -translate-y-1/2 text-[#9FB0C4] hover:text-[#0077CC]"
            >
      ...
      <nav className="sticky top-[136px] z-30 bg-[#0A1F44]/90 backdrop-blur-md py-4 -mx-6 px-6 md:-mx-16 md:px-16 flex overflow-x-auto hide-scrollbar gap-6 sm:gap-8 items-center border-b border-[#0077CC]/15">
        {categories.map((cat) => {
      ...
      <section className="mt-8 sm:mt-12">
        <div className="flex items-center justify-between mb-8">
          <h2 className="font-serif-display font-semibold text-2xl sm:text-3xl text-[#5C9FD6]">
            {selectedCategory === 'All' ? t.allSelection : (selectedCategory === 'Plov' ? t.masterPlovSelection : selectedCategory)}
          </h2>
          <div className="h-px bg-[#0077CC]/20 flex-grow ml-6 hidden sm:block" />
        </div>

        {/* Dish Grid */}
        {filteredDishes.length === 0 ? (
          <div className="glass-card rounded-2xl p-12 text-center border border-[#0077CC]/30 max-w-md mx-auto my-12">
            <span className="material-symbols-outlined text-[#0077CC] text-5xl mb-4">
              restaurant_menu
            </span>
            <h3 className="font-serif-display font-semibold text-2xl text-[#0077CC] mb-2">
              Hozircha taomlar mavjud emas
            </h3>
            <p className="font-sans-body text-xs text-[#9FB0C4] opacity-80">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
            {filteredDishes.map((dish) => {
              ...
              <div
                ...
                className="glass-card rounded-2xl overflow-hidden group flex flex-col h-full border border-[#0077CC]/30 shadow-xl cursor-pointer glass-card-hover"
              >
                {/* Photo Header */}
                <div className="relative h-64 overflow-hidden">
                  <div className="absolute top-4 right-4 z-10">
                    <button
                      ...
                      className="bg-[#0A1F44]/60 backdrop-blur-md p-2 rounded-full text-[#0077CC] hover:scale-110 transition-transform active:scale-95"
                      aria-label="Favorite"
                    >
                      <span
                  ...
                  />
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#0A1F44] to-transparent h-20" />
                </div>

                {/* Content */}
                <div className="p-6 flex flex-col flex-grow">
                  <div className="flex items-baseline mb-2">
                    <h3 className="font-serif-display font-semibold text-xl text-[#FFFFFF] group-hover:text-[#0077CC] transition-colors">
                      {dish.name}
                    </h3>
                    <span className="dotted-leader" />
                    <span className="font-serif-display font-bold text-lg text-[#0077CC] whitespace-nowrap">
                      {dish.priceUZS.toLocaleString()} UZS
                    </span>
                  </div>

                  <p className="font-sans-body text-xs text-[#9FB0C4] mb-6 leading-relaxed line-clamp-2">
                    {dish.description}
                  </p>
                  ...
                  <div className="mt-auto flex items-center justify-between pt-4 border-t border-[#0077CC]/15">
                    ...
                    <button
                      onClick={() => openDishDetail(dish)}
                      className="px-5 py-2 rounded-full bg-[#0077CC]/15 text-[#0077CC] border border-[#0077CC]/40 font-sans-body text-xs font-bold tracking-widest hover:bg-[#0077CC] hover:text-white transition-all active:scale-95 uppercase flex items-center gap-1.5"
                    >
                      <span>{t.details}</span>
